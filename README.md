...
3. [Generate an API Key from the HERE Developer Portal](#3-generate-an-api-key-from-the-here-developer-portal).
...
4. [Add a dialog skill](https://cloud.ibm.com/docs/assistant?topic=assistant-skill-dialog-add) to the **Assistant** by importing the [`starter-kit-cooperation-dialog-skill.json`](./starter-kit/assistant/starter-kit-cooperation-dialog-skill.json) file.
5. Go back to All Assistants page, open **Settings** from the action menu ( **`⋮`** ) and click on **API Details**.
6. Note the **Assistant ID**, **API Key** and **Assistant URL**. For **Assistant URL**, make note of the base URL/domain (e.g., `https://api.us-south.assistant.watson.cloud.ibm.com` or `https://api.eu-gb.assistant.watson.cloud.ibm.com`) and not the full directory/path. You will need all 3 of these values in Step 4, below.
...
4. Edit the **name** value in the `manifest.yml` file to your application name (for example, _my-app-name_).
5. From a terminal:
    i. Go to the `starter-kit/server-app` directory of the cloned repo.
    ii. Install the dependencies: `npm install`.
    iii. Launch the server application locally or deploy to IBM Cloud:
        - To run locally:
            a. Start the application: `npm start`.
            b. The server can be accessed at http://localhost:3000.
        - To deploy to IBM Cloud:
            a. Log in to your IBM Cloud account using the IBM Cloud CLI: `ibmcloud login`.
            b. Target a Cloud Foundry org and space: `ibmcloud target --cf`.
            c. Push the app to IBM Cloud: `ibmcloud app push`.
            d. The server can be accessed at a URL using the **name** given in the `manifest.yml` file (for example,  https://my-app-name.bluemix.net).
...
4. From a terminal:
    i. Go to the `starter-kit/mobile-app` directory.
    ii. Install the dependencies: `npm install`.
    iii. Go to the `ios` directory: `cd ios`.
    iv. Install pod dependencies: `pod install`.
    v. Return to the `mobile-app` directory: `cd ../`.
    vi. Launch the app in the simulator: `npm run ios`.
